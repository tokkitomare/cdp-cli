...
pub fn parse_response(arg: String) -> Result<(), CliErr> {
    ...
    if arg.starts_with("(") && arg.ends_with(")") {
        let path = create_cdp_cmdaliases()?;
            ...
            return Err(
                CliErr::set_err(
                    &format!(
                        ...
                        "(<ID: current ALIAS>: <NEW_ALIAS: type the new alias or just type '.' to leave unchanged>;<NEW_CMD: type the new command or . to leave unchanged>)".green()
                    ), 
                    ErrKind::InvalidData
                )
            );
